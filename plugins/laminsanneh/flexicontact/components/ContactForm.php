...
class ContactForm extends ComponentBase{
    ...
    public function onMailSent(){
        ...
        $errorHappened = false;
        if (empty($name) || empty($fromEmail) || empty($phone) ){
            $errorHappened = true;
            $flashMessae = '';
            if ( empty($name) )
                $flashMessae .= '<p>Пожалуйста укажите имя.</p>';
            if ( empty($fromEmail) )
                $flashMessae .= '<p>Пожалуйста укажите E-Mail.</p>';
            if ( empty($phone) )
                $flashMessae .= '<p>Пожалуйста укажите телефон.';
            // if ( empty($body) )
        ...
        $data = compact('phone','email','name');

        $this->page["confirmation_text"] = Settings::get('confirmation_text');
        if ($errorHappened)
            return ['error' => true, 'message' => 'Не пройдена валидация'];
        else
